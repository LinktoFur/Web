...
const Markdown: React.FC<MarkdownProps> = ({ content, className = "", basePath = "", onInternalNav, onOpenRules }) => {
  // Pre-processing
      ...
      <ReactMarkdown
        ...
        components={{
          a: ({ node, ...props }) => {
            ...
            const isRulesLink = rawHref.includes('docs.qq.com/form/page/DVVZXV09zUUdDUWJs');

            if (onOpenRules && isRulesLink) {
              return (
                <a
                  {...props}
                  href={rawHref}
                  className="text-brand-600 dark:text-brand-400 hover:underline cursor-pointer font-medium"
                  onClick={(e) => {
                    e.preventDefault();
                    onOpenRules();
                  }}
